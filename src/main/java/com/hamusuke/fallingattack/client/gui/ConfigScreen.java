...
public class ConfigScreen extends Screen {
    ...
    protected void init() {
        this.addRenderableWidget(new Button((this.width / 2) - (this.width / 4), this.height / 2 - 10, this.width / 2, 20, CommonComponents.optionStatus(new TranslatableComponent("options.fallingattack.third.person"), Config.Client.THIRD_PERSON.get()), p_onPress_1_ -> {
            Config.Client.THIRD_PERSON.set(!Config.Client.THIRD_PERSON.get());
            p_onPress_1_.setMessage(CommonComponents.optionStatus(new TranslatableComponent("options.fallingattack.third.person"), Config.Client.THIRD_PERSON.get()));
        }, (p_onTooltip_1_, p_onTooltip_2_, p_onTooltip_3_, p_onTooltip_4_) -> {
            this.renderTooltip(p_onTooltip_2_, new TranslatableComponent("options.fallingattack.third.person.desc"), p_onTooltip_3_, p_onTooltip_4_);
        }));
